...
const int MotorRdir = 52;  // 52    //Right motor Direction Control pin
const int MotorLdir = 53;  // 53    //Left motor Direction Control pin
const int MotorRpwm = 4;  // 4     //Right motor PWM Speed Control pin
const int MotorLpwm = 5;  // 5     //Left motor PWM Speed Control pin
...
double omegaLeft = 0;
double R = 0.068;
double L = 0.273; // 0.275;  // 0.3015;  //0.275  //0.351
double V = 0;
double omega = 0;
double Vl = 0;
double Vr = 0;
...
void printValue(double val, const char* namVal, bool withSignAndDouble) {
  Serial1.print(namVal==NULL ? "" : String(namVal) + ": ");

  if (withSignAndDouble) {
    Serial1.print(val>=0 ? "+" : "");
    Serial1.print(val);
  } else {
    Serial1.print(int(val));
  }
